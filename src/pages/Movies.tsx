...
import { Film, ChevronDown, List, Grid3X3 } from 'lucide-react';
...
const Movies = () => {
  ...
  const handleTabChange = (tab: 'popular' | 'top_rated') => {
    setActiveTab(tab);
    trackMediaPreference('Movies Tab', tab);
  };
  ...
  const filterByGenre = (movies: Media[] = []) => {
    if (genreFilter === 'all') return movies;
    return movies.filter((movie) => movie.genre_ids.includes(Number(genreFilter)));
  };

  const filteredPopularMovies = sortMovies(filterByGenre(popularMovies?.results));
  const filteredTopRatedMovies = sortMovies(filterByGenre(topRatedMovies?.results));

  const hasMorePopular = popularMovies && popularPage < popularMovies.total_pages;
  const hasMoreTopRated = topRatedMovies && topRatedPage < topRatedMovies.total_pages;
