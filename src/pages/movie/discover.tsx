...
import { MOVIE_GENRES } from '@/utils/services/movie-genres';
import { MOVIE_LANGUAGES } from '@/utils/services/movie-languages';
import { MOVIE_PROVIDERS } from '@/utils/services/movie-providers';
	...
	useEffect(() => {
		async function fetchMovies() {
			...
			const params: DiscoverMoviesParams = {
				...
				'primary_release_date.gte': fromYear ? `${fromYear}-01-01` : undefined,
				'primary_release_date.lte': toYear ? `${toYear}-12-31` : undefined,
				...
				<div className="max-w-7xl mx-auto px-4 py-8 w-full flex-1">
					<div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-4">
						<div className="text-left">
							<h1 className="text-3xl md:text-4xl font-bold text-white text-left">Discover Movies</h1>
							<p className="text-gray-300 mt-1 text-base max-w-2xl text-left">
								Find movies by genre, rating, year, and more. Use the filters and sorting options to explore TMDB’s vast movie collection.
							</p>
						</div>
						<div className="flex gap-2 items-center">
							<Sheet open={filterOpen} onOpenChange={setFilterOpen}>
								...
								<SheetContent side="right" className="fixed z-50 gap-4 bg-background p-6 shadow-lg transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500 inset-y-0 right-0 h-full w-3/4 border-l data-[state=closed]:slide-out-to-right data-[state=open]:slide-in-from-right sm:max-w-sm flex flex-col px-0">
									<div className="flex flex-col space-y-2 text-center sm:text-left px-4 md:px-6">
										...
										<p className="text-sm text-muted-foreground">Narrow down your search results with the following filters.</p>
									</div>
									<div className="relative overflow-hidden px-4 md:px-6" style={{ position: 'relative' }}>
										<div className="space-y-8">
											{/* Genres */}
											<div className="space-y-2">
												<label className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 text-muted-foreground">Genres</label>
											...
											<div className="grid gap-2 md:grid-cols-2">
												<div className="space-y-2">
													<label className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 flex text-muted-foreground">From</label>
													<select
												...
												<div className="space-y-2">
													<label className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 flex text-muted-foreground">To</label>
													<select
														className="inline-flex items-center gap-2 whitespace-nowrap rounded-md text-sm ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 border border-input bg-background hover:bg-accent hover:text-accent-foreground h-10 px-4 py-2 w-full justify-start text-left font-normal text-muted-foreground"
														value={toYear || ''}
											...
											{/* Language */}
											<div className="space-y-2">
												<label className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 flex text-muted-foreground">Language</label>
												<select
											...
											{/* Provider */}
											<div className="space-y-2">
												<label className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 flex items-center gap-2 text-muted-foreground">Where to watch
											...
											<div className="space-y-4">
												<label className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 text-muted-foreground">Vote Average</label>
												<Slider
													min={0}
													max={10}
													step={0.1}
											...
											{/* Minimum Votes */}
											<div className="space-y-4">
												<label className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 text-muted-foreground">Minimum Votes</label>
												<Slider
													min={0}
													max={500}
													step={10}
							...
							<Popover>
								...
								<PopoverContent align="end" side="top" className="z-50 w-72 rounded-md border bg-popover text-popover-foreground shadow-md outline-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 flex flex-col gap-1 p-1">
									{SORT_OPTIONS.map(opt => (
										<button
											key={opt.value}
											className={`inline-flex items-center gap-2 whitespace-nowrap rounded-md text-sm ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0 ${sortBy === opt.value ? 'bg-primary text-primary-foreground hover:bg-primary/90' : 'hover:bg-accent hover:text-accent-foreground'} h-10 px-4 py-2 justify-between text-left font-normal`}
