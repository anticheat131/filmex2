...
import { getMediaDetails } from '@/utils/services/tmdb';
...
const ContentRow = ({ title, media, featured = false }: ContentRowProps) => {
  ...
  useEffect(() => {
    const enrichWithRuntime = async () => {
      const enriched = await Promise.all(
        applyQuality(media).map(async (item) => {
          if (item.media_type === 'movie' && item.runtime == null) {
            try {
              const details = await getMediaDetails('movie', item.id);
