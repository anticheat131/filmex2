...
const fetchLogo = async (movieId) => {
  const res = await fetch(`https://api.themoviedb.org/3/movie/${movieId}/images?api_key=${TMDB_API_KEY}`);
  const data = await res.json();
  let logo = null;
...
const TrendingToday = () => {
  ...
  useEffect(() => {
    fetchTrendingToday().then(async (movies) => {
      ...
      await Promise.all(movies.map(async (m) => {
        const logo = await fetchLogo(m.id);
        if (logo) logoMap[m.id] = logo;
      }));
      setLogos(logoMap);
    });
  }, []);
  ...
  return (
    <section className="w-full mt-8 p-0 m-0">
      <div className="grid grid-cols-1 grid-rows-4 md:grid-cols-2 md:grid-rows-2 gap-y-10 gap-x-[2vw] md:gap-[2vw] w-full h-[440vw] md:h-[72vw] max-h-[140vh]">
        {movies.map((movie, idx) => (
          <div key={movie.id} className="relative bg-black/80 overflow-hidden shadow-2xl flex flex-col justify-center items-center w-full h-full min-h-[90vw] min-w-0 group flex-1 rounded-xl pb-10">
            <img
            ...
            <div className="relative z-20 flex flex-col items-center justify-end px-2 md:px-4 pt-2 md:pt-4 pb-6 md:pb-10 w-full h-full">
              ...
              <div className="flex gap-2 md:gap-4 w-full justify-center mt-1 md:mt-2">
                ...
                <button
                  ...
                  onClick={() => navigate(`/movie/${movie.id}?play=1`)}
                >
